...
#ifdef HAVE_BMP280 
	static uint8_t bmp280Output[2 * 3];  //two bmp280 output measures
	static uint8_t bmp280Count;
#else
	static int8_t ms5611Step;
	static uint8_t ms5611Output[3 * 3];  //three ms5611 output measures
	static uint8_t ms5611Count;
#endif
#ifdef HAVE_ACCELEROMETER
	static uint8_t checkOutput[2];
	static uint8_t imuOutput[LIGHT_INVENSENSE_COMPRESSED_DMP_PAQUET_LENGTH]; //imu dmp fifo output
	static uint8_t imuCount;
#ifdef AK89xx_SECONDARY
	static uint8_t magOutput[8];    //magnetometer output
	static uint8_t magCount;
#endif //AK89xx_SECONDARY
#endif //HAVE_ACCELEROMETER
