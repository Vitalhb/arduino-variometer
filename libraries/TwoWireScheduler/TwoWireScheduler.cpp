...
/* static class data */
/*********************/
uint8_t TWScheduler::status = 0;   //no problem to not release at start as there is no values
#ifdef HAVE_BMP280 
uint8_t TWScheduler::bmp280Output[2 * 3];  //two bmp280 output measures
uint8_t TWScheduler::bmp280Count = TWO_WIRE_SCHEDULER_BMP280_SHIFT;
#else
int8_t TWScheduler::ms5611Step = 0;
uint8_t TWScheduler::ms5611Output[3 * 3];  //three ms5611 output measures
uint8_t TWScheduler::ms5611Count = TWO_WIRE_SCHEDULER_MS5611_SHIFT;
#endif
#ifdef HAVE_ACCELEROMETER
uint8_t TWScheduler::checkOutput[2];
uint8_t TWScheduler::imuOutput[LIGHT_INVENSENSE_COMPRESSED_DMP_PAQUET_LENGTH]; //imu dmp fifo output
uint8_t TWScheduler::imuCount = TWO_WIRE_SCHEDULER_IMU_SHIFT;
#ifdef AK89xx_SECONDARY
uint8_t TWScheduler::magOutput[8];    //magnetometer output
uint8_t TWScheduler::magCount = TWO_WIRE_SCHEDULER_MAG_SHIFT;
